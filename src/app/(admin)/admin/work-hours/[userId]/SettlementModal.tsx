...
export default function SettlementModal({ isOpen, onClose, user, shifts, period }: Props) {
        ...
        return (
            <div className="fixed inset-0 z-50 overflow-y-auto bg-black/70 flex items-start justify-center p-4 pt-8 backdrop-blur-sm">
                <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden border border-gray-100">
                    ...
                    {/* Preview Body — styled to match the PDF layout */}
                        ...
                        <div className="border border-gray-200 rounded-lg overflow-hidden">
                            <div className="divide-y divide-gray-100">
                                ...
                                <div className="flex justify-between px-4 py-2.5 text-sm bg-gray-50">
                                    ...
                                    <span className="font-bold text-gray-900">${totalSalary.toFixed(2)}</span>
                                </div>
                                {reviewBonus > 0 && (
                                    <div className="flex justify-between px-4 py-2.5 text-sm">
                                        <span className="text-gray-600">Review Bonus</span>
                                        <span className="font-semibold text-green-600">+ ${reviewBonus.toFixed(2)}</span>
                                    </div>
                                )}
                                <div className="flex justify-between px-4 py-2.5 text-sm">
                                    <span className="text-gray-600">Cash Deduction (Net)</span>
                                    <span className="font-semibold text-red-600">− ${netCashDeduction.toFixed(2)}</span>
                                </div>
                                <div className="px-4 py-1 text-xs text-gray-400 bg-gray-50">
                                    (Total Cash ${totalCash} − Store Spend ${totalSpend})
                                </div>
                            </div>
                            {/* Total Due Box */}
                            <div className="bg-blue-600 px-4 py-4 flex justify-between items-center">
                                ...
                                <span className={`font-black text-2xl ${totalDue >= 0 ? 'text-white' : 'text-red-300'}`}>
                    ...
                    <div className="px-6 py-4 border-t bg-gray-50 flex gap-3">
                        <button
                            onClick={() => setShowPreview(false)}
                            className="flex-1 py-3 px-4 text-sm font-bold text-gray-600 border border-gray-200 rounded-xl hover:bg-gray-100 transition-colors"
                        >
                            ← Edit
                        </button>
                        <button
                            ...
                            className="flex-[2] bg-blue-600 text-white py-3 px-4 rounded-xl font-bold text-sm shadow-md hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest disabled:opacity-60"
                        >
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden border border-gray-100">
                ...
                <div className="px-6 py-5 border-b bg-gray-50 flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-0.5">{user.name} · {dayjs.utc(period.startDate).format('MMM D')} - {dayjs.utc(period.endDate).format('MMM D, YYYY')}</p>
                    </div>
                    <button onClick={onClose} className="text-gray-400 hover:text-gray-600 transition-colors">
                ...
                <div className="p-6 space-y-6">
                    {/* Hero Stats */}
                    <div className="grid grid-cols-2 gap-4 bg-blue-50 rounded-xl p-4 border border-blue-100">
                        <div>
                            <p className="text-xs font-bold text-blue-600 uppercase tracking-wider">Calculated Hours</p>
                            <p className="text-2xl font-black text-blue-900">{totalHours.toFixed(2)}h</p>
                        </div>
                        <div className="text-right">
                            <p className="text-xs font-bold text-blue-600 uppercase tracking-wider">Gross Salary</p>
                            <p className="text-2xl font-black text-blue-900">${totalSalary.toLocaleString(undefined, { minimumFractionDigits: 2 })}</p>
                        </div>
                    </div>
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 border border-gray-200 rounded-lg px-4 py-2.5 text-sm font-semibold text-gray-900 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all"
                                />
                            </div>
                            <div>
                    ...
                    <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl border-2 border-dashed border-indigo-200 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 hover:border-indigo-300 transition-all group"
                    >
                        {isAnalyzing ? (
                    ...
                    {/* Final Result */}
                    <div className="bg-gray-900 rounded-xl p-6 text-white text-center shadow-lg relative overflow-hidden">
                        ...
                        <div className="mt-4 flex flex-wrap justify-center gap-x-4 gap-y-1 text-[10px] text-gray-500 font-medium uppercase tracking-widest">
                ...
                <div className="px-6 py-4 border-t bg-gray-50 flex flex-col sm:flex-row gap-3">
                    <button onClick={onClose} className="flex-1 py-3 px-4 text-sm font-bold text-gray-600 hover:text-gray-900 transition-colors uppercase tracking-wider">
                        Cancel
                    </button>
                    <button
                        onClick={() => setShowPreview(true)}
                        className="flex-[2] bg-blue-600 text-white py-3 px-4 rounded-xl font-bold text-sm shadow-md hover:bg-blue-700 active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-widest"
                    >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
